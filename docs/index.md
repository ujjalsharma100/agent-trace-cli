...
| Line-level AI vs no-attribution labels | [`agent-trace blame`](reference/blame.md) |
...
- **Ledger-only blame** — `blame` does not guess from heuristics. Attribution is binary (**AI** or **No attribution**); if there is no ledger (and no usable inline data in a git note), lines are **No attribution**.
