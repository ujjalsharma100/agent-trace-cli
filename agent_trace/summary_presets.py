...
def _build_prompt(transcript_text: str) -> str:
    ...
    if not t:
        return ""
    return (
        "You are summarizing a coding-assistant conversation transcript. Readers will mostly infer what "
        "changed from the repository and diffs; this summary should capture what is easy to miss when "
        "only looking at code.\n\n"
        "Weight your answer toward the human/user side of the transcript: stated goals, intent, "
        "constraints, preferences, definitions, repro steps, URLs or identifiers, timelines, tradeoffs, "
        "corrections, and explicit decisions. Those signals often do not survive in the final code.\n\n"
        "Mention what the assistant did or concluded only briefly and only when it clarifies something "
        "that is not obvious from the edits alone. Skip long inventories of tools, files, or patches "
        "unless the user asked for something specific about them.\n\n"
        "Stay factual and tight: short paragraphs or a few bullets, no rigid section template. Avoid markdown code fences.\n\n"
        "Transcript:\n"
        f"{t}\n"
    )
